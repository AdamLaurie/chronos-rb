/**
 * CHRONOS-Rb GNSS Input Module
 *
 * Handles GNSS receiver input (u-blox NEO-M8N, M9N, or similar) for:
 * - Backup time source when rubidium is unavailable
 * - Initial time setting before rubidium locks
 * - Position information for PTP/NTP
 *
 * Connections:
 *   GNSS TX  -> GP5 (UART1 RX)
 *   GNSS RX  <- GP4 (UART1 TX, for UBX commands)
 *   GNSS PPS -> GP11
 *   GNSS VCC -> 3.3V
 *   GNSS GND -> GND
 *
 * Copyright (c) 2025 - Open Source Hardware Project
 * License: MIT
 */

#ifndef GNSS_INPUT_H
#define GNSS_INPUT_H

#include <stdint.h>
#include <stdbool.h>

/*============================================================================
 * GNSS STATUS STRUCTURES
 *============================================================================*/

/* GNSS fix type */
typedef enum {
    GNSS_FIX_NONE = 0,      /* No fix */
    GNSS_FIX_2D = 2,        /* 2D fix (no altitude) */
    GNSS_FIX_3D = 3         /* 3D fix */
} gnss_fix_type_t;

/* GNSS time data */
typedef struct {
    uint16_t year;          /* UTC year (e.g., 2025) */
    uint8_t month;          /* UTC month (1-12) */
    uint8_t day;            /* UTC day (1-31) */
    uint8_t hour;           /* UTC hour (0-23) */
    uint8_t minute;         /* UTC minute (0-59) */
    uint8_t second;         /* UTC second (0-59) */
    uint16_t millisecond;   /* Milliseconds (0-999) */
    bool valid;             /* Time data valid */
} gnss_time_t;

/* GNSS position data */
typedef struct {
    double latitude;        /* Latitude in degrees (+ = N, - = S) */
    double longitude;       /* Longitude in degrees (+ = E, - = W) */
    double altitude;        /* Altitude in meters (MSL) */
    double speed_knots;     /* Speed over ground in knots */
    double course;          /* Course over ground in degrees */
    double hdop;            /* Horizontal dilution of precision */
    bool valid;             /* Position data valid */
} gnss_position_t;

/* Complete GNSS state */
typedef struct {
    gnss_time_t time;        /* UTC time from GNSS */
    gnss_position_t position; /* Position from GNSS */
    gnss_fix_type_t fix_type; /* Current fix type */
    uint8_t satellites;      /* Number of satellites in use */
    uint8_t satellites_view; /* Number of satellites in view */
    bool pps_valid;          /* PPS signal present and valid */
    uint64_t last_pps_us;    /* Timestamp of last PPS (us since boot) */
    uint64_t last_nmea_us;   /* Timestamp of last valid NMEA */
    uint32_t pps_count;      /* Total PPS pulses received */
    uint32_t nmea_count;     /* Total NMEA sentences parsed */
    uint32_t nmea_errors;    /* NMEA parse errors */
} gnss_state_t;

/*============================================================================
 * FUNCTION PROTOTYPES
 *============================================================================*/

/* Initialization */
void gnss_input_init(void);

/* Task - call from main loop */
void gnss_input_task(void);

/* Status queries */
bool gnss_has_fix(void);
bool gnss_has_time(void);
bool gnss_pps_valid(void);
uint8_t gnss_get_satellites(void);
gnss_fix_type_t gnss_get_fix_type(void);

/* Time access */
uint32_t gnss_get_unix_time(void);
void gnss_get_utc_time(gnss_time_t *time);
uint64_t gnss_get_last_nmea_us(void);

/* PPS access */
uint64_t gnss_get_last_pps_us(void);
uint32_t gnss_get_pps_count(void);

/* Position access */
void gnss_get_position(double *lat, double *lon, double *alt);
void gnss_get_position_full(gnss_position_t *pos);

/* Full state access */
const gnss_state_t* gnss_get_state(void);

/* Control */
void gnss_enable(bool enable);
bool gnss_is_enabled(void);
void gnss_reset_time(void);
void gnss_set_debug(bool enable);
bool gnss_get_debug(void);

/* Module info */
const char* gnss_get_firmware_version(void);
const char* gnss_get_hardware_version(void);
int8_t gnss_get_leap_seconds(void);
bool gnss_leap_seconds_is_valid(void);

#endif /* GNSS_INPUT_H */
